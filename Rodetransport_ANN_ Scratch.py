...
plt.rcParams['font.sans-serif']=['SimHei']  # 使plt中的中文字体能够显现
plt.rcParams['axes.unicode_minus']=False    # 使plt中的中文字体能够显现
...
# 输入数据
x_data = np.array([sqrs,sqjdcs,sqglmj])
x_train = x_data[:,0:15]    # 前15个数据作为训练集
x_test = x_data[:,15:20]    # 后5个数据作为测试集
# 真实值
t_data = np.array([glkyl,glhyl])
t_train = t_data[:,0:15]
t_test = t_data[:,15:20]
